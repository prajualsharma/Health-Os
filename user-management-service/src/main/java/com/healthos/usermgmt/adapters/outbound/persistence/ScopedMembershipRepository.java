...
public interface ScopedMembershipRepository extends JpaRepository<ScopedMembership, UUID> {
  List<ScopedMembership> findByAccountIdAndStatus(UUID accountId, MembershipStatus status);
  ...
  Optional<ScopedMembership> findByAccountIdAndPortalTypeAndScopeTypeAndScopeIdAndRoleName(
      UUID accountId, PortalType portalType, ScopeType scopeType, UUID scopeId, String roleName);
}
